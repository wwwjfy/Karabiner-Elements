...
public:
  ...
  void handle_keyboard_event(device_registry_entry_id device_registry_entry_id,
    ...
    if (process_standalone_key_key(to_key_code, keyboard_type, pressed)) {
      key_repeat_manager_.stop();
      return;
    }
...
private:
  ...
  bool process_standalone_key_key(krbn::key_code key_code, krbn::keyboard_type keyboard_type, bool pressed) {
    if (pressed) {
      if (!standalone_keys_.get(key_code) || (standalone_keys_timer_ != nullptr && key_code != standalone_key_)) {
        if (standalone_keys_timer_ != nullptr) {
          if (!post_modifier_flag_event(standalone_key_, keyboard_type, pressed)) {
            post_key(standalone_key_, standalone_key_, keyboard_type, true, false);
          }
      ...
      if (key_code == standalone_key_ && standalone_keys_timer_ != nullptr) {
        standalone_keys_timer_ = nullptr;
        post_standalone_key_key(standalone_key_, keyboard_type);
        return true;
      } else {
        return false;
      }
  ...
  bool post_standalone_key_key(krbn::key_code key_code, krbn::keyboard_type keyboard_type) {
